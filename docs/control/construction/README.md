...
As stated above, most Lithium Ion USB power-banks will shut-off early, even when the camera still needs the power. A select few USB battery sources include an "always on feature" designed for time-lapse projects.  Example: https://voltaicsystems.com/v50/  This one can even solar recharge the battery at the same time.
